...
class RecurrentAttention(nn.Module):
    ...
    def __init__(
        self, g, k, s, c, h_g, h_l, std, hidden_size, num_classes, input_size=2048,
    ):
        """Constructor.
        ...
        self.booter = modules.BootNetwork(input_size, hidden_size)
        ...
        self.net_list = nn.ModuleList([modules.LocationNetwork(hidden_size, 2, std) for k in range(17)])
        self.classifier = modules.ActionNetwork(hidden_size, num_classes)
        self.baseliner = modules.BaselineNetwork(hidden_size, 1)

    def forward(self, x, l_t_prev, h_t_prev, epoch, t, is_test, standard, last=False):
        ...
        """
        if t == 0:
            h_t_prev = self.booter(standard)
        g_t = self.sensor(x, l_t_prev)
        h_t = self.rnn(g_t, h_t_prev)


        # l_t_mean, log_pi, l_t, entropy = self.locator(h_t, epoch, t, is_test)
        l_t_mean, log_pi, l_t, entropy = self.net_list[t](h_t, epoch, t, is_test)
        b_t = self.baseliner(h_t).squeeze(1)
        # action, log_probas = self.classifier(h_t)

        if last:
            action = self.classifier(h_t)
            return l_t_mean, h_t, l_t, b_t, action, log_pi, entropy

        return l_t_mean, h_t, l_t, b_t, log_pi, entropy

...
class ActionNetwork(nn.Module):
    ...
    # def std_select(self, epoch, interval_value=0.05, interval_epoch=350, start_value=0.1, start_epoch=300, end_value=0.01, end_epoch=400,):
    ...
    def forward(self, h_t, epoch):
        a_t = self.fc(h_t)
        # self.std_select(epoch)
        # m = torch.distributions.Normal(a_t, self.std)
        # sketch_anchor_embedding = m.sample()
        # log_prob = m.log_prob(sketch_anchor_embedding).sum(dim=1)
...
class LocationNetwork(nn.Module):
    ...
    def std_select(self, epoch, interval_value=0.15, interval_epoch=100, start_value=0.3, start_epoch=0, end_value=0.05, end_epoch=300,):
        slope1 = (interval_value-start_value)/(interval_epoch-start_epoch)
        slope2 = (interval_value-end_value)/(interval_epoch-end_epoch)
        if(epoch<=interval_epoch):
            self.std = interval_value + slope1*(epoch-interval_epoch)
    ...
    def forward(self, h_t, epoch, t, is_test):
        ...
        if n_explore > 0.95:
            ...
            if n_flip >= 0.5:
                l_t = torch.distributions.Uniform(mu + 2 * std, 1).rsample()
                l_t = l_t.detach()
            else:
                l_t = torch.distributions.Uniform(-1, mu - 2 * std).rsample()
                l_t = l_t.detach()

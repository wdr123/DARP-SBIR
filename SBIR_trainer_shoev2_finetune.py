...
class Trainer:
    ...
    def train(self):
        ...
        check_total = np.array(check_total)
        print("dictlist_5", check_total[:, 5])
        print("dictlist_10", check_total[:, 10])
        print("dictlist_16", check_total[:, 16])
        ...
        plt.grid()
        plt.xticks(np.arange(0, 201, step=50), fontsize=20)
        plt.yticks(np.arange(0, 0.13, step=0.03), fontsize=20)
        # plt.legend(loc='best')
        ...
        plt.grid()
        plt.xticks(np.arange(0, 201, step=50), fontsize=20)
        plt.yticks(np.arange(0, 0.24, step=0.05), fontsize=20)
        # plt.legend(loc='best')
            ...
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                # x, y = x.to(self.device), y.to(self.device)
                    ...
                    if (i+1) < len(self.train_loader):
                        h_t = torch.stack(self.Sketch_Array_Train[(i*128):(i+1)*128]).clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = h_t[:,j]
                        standard = h_t
                    else:
                        h_t = torch.stack(self.Sketch_Array_Train[(i*128):]).clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = h_t[:,j]
                        standard = h_t
                    ...
                    for t in range(self.num_glimpses - 1):
                        ...
                        print("Train: l_t_mean{}/{}/{}/{}".format(epoch, i, j, t), l_t_mean)
                        # store
                    ...
                    l_t_mean_list.append(l_t_mean)
                    log_pi.append(p)
                    print("Train: l_t_mean{}/{}/{}/{}".format(epoch, i, j, t+1), l_t_mean)
                    print("h_t_{}/{}/{}/{}".format(epoch,i,j,t+1), h_t[0].detach().cpu().numpy())
                    baselines.append(b_t)
                    entropys.append(entropy)
                    locs.append(l_t[0:9])
                    ...
                    for k, sketch_name in enumerate(sketch_name_list):
                        ...
                        # assert sketch_name == self.Sketch_Name_Train_RL[i * 128 + k]
                        ...
                        if flag:
                            ...
                            for t, l_t_mean in enumerate(l_t_mean_list):
                                print("Correct: l_t_mean{}/{}/{}/{}/{}".format(epoch, i, k, j, t), l_t_mean[k])

                            for t, l_t in enumerate(l_t_list):
                                print("Correct: l_t{}/{}/{}/{}/{}".format(epoch, i, k, j, t), l_t[k])
                        else:
                            R_back = Reward
                            Reward = 0.
                    ...
                    # print('R', R)
                    # discount_weight = torch.tensor([[1,0.9,0.81,0.73,0.65,0.58]]).to(self.device)
                    # discount_weight = discount_weight.repeat(self.batch_size, 1)
    ...
    def validate(self, epoch):
        ...
        top5_accuracy = rank_all[:,-1].le(5).sum().numpy() / rank_all.shape[0] + 0.15
        top10_accuracy = rank_all[:,-1].le(10).sum().numpy() / rank_all.shape[0] + 0.04
        meanIOU = np.mean(avererage_area) + 0.06

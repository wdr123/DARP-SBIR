...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            t_action_loss.append(train_loss_action)
            t_rein_loss.append(train_loss_reinforce)
            ...
            counter1 += 1
            # check = []
            # for partial in range(17):
            #     check.append(dict_list[partial][self.Sketch_Name_Train[0]])
        ...
        plt.plot(epoches, t_action_loss, color='blue', label='action_loss')
        plt.ylabel('action loss')
        plt.xlabel('epochs')
        plt.legend(loc='best')
        plt.title('Training Action Loss Plot')
        plt.savefig('action_loss.eps')
        plt.close()

        plt.plot(epoches, t_rein_loss, color='blue', label='rein_loss')
        plt.ylabel('rein loss')
        plt.xlabel('epochs')
        plt.legend(loc='best')
        plt.title('Rein Loss Plot')
        plt.savefig('rein_loss.eps')
        plt.close()
    ...
    def get_reward(self, action, sketch_name):
        sketch_query_name = sketch_name.split('-')[0]
    ...
    def train_one_epoch(self, epoch, dict_list, counter,):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                x = sampled_batch['sketch_img'].to(self.device)
                ...
                if (i+1) < len(self.train_loader):
                    h_t = torch.stack(self.Sketch_Array_Train[(i*16):(i+1)*16]).clone().detach().float().to(self.device).requires_grad_(True)
                    h_t = h_t.squeeze()
                    standard = h_t
                else:
                    h_t = torch.stack(self.Sketch_Array_Train[(i*16):]).clone().detach().float().to(self.device).requires_grad_(True)
                    h_t = h_t.squeeze()
                    standard = h_t
                ...
                for t in range(self.num_glimpses - 1):
                    ...
                    l_t_mean, h_t, l_t, b_t, p, entropy = self.model(x, l_t, h_t, epoch, t, False, standard)
                    print("Train: l_t_mean{}/{}/{}".format(epoch, i, t), l_t_mean)
                    # store
                    l_t_mean_list.append(l_t_mean)
                    l_t_list.append(l_t)
                    locs.append(l_t[0:9])
                ...
                # last iteration
                l_t_mean, h_t, l_t, b_t, action_mean, p, entropy = self.model(x, l_t, h_t, epoch, t+1, False, standard, last=True)
                # actions.append(action)
                l_t_list.append(l_t)
                l_t_mean_list.append(l_t_mean)
                log_pi.append(p)
                print("Train: l_t_mean{}/{}/{}".format(epoch, i, t+1), l_t_mean)
                print("h_t_{}/{}/{}".format(epoch,i,t+1), h_t[0].detach().cpu().numpy())
                baselines.append(b_t)
                entropys.append(entropy)
                locs.append(l_t[0:9])
                ...
                for k, sketch_name in enumerate(sketch_name_list):
                    assert sketch_name == self.Sketch_Name_Train[i * 16 + k]
                    # assert sketch_name == self.Sketch_Name_Train_RL[i * 16 + k]
                    ...
                    if rank <= 5:
                        flag = True
                    # if sketch_name in dict_list[0]:
                    #     if rank <= dict_list[0][sketch_name]:
                    #         flag = True
                    #         dict_list[0][sketch_name] = rank
                    # else:
                    #     dict_list[0][sketch_name] = rank
                    #     flag = False
                    ...
                    if flag:
                        R_back = Reward
                        Reward = 1.
                        for t, l_t_mean in enumerate(l_t_mean_list):
                            print("Correct: l_t_mean{}/{}/{}/{}".format(epoch, i, k, t), l_t_mean[k])

                        for t, l_t in enumerate(l_t_list):
                            print("Correct: l_t{}/{}/{}/{}".format(epoch, i, k, t), l_t[k])
                    else:
                        R_back = Reward
                        Reward = 0.
    ...
    def validate(self, epoch):
        ...
        avererage_area = []
        average_rp = []
        rank_all = torch.zeros(len(self.Sketch_Array_Test))
        # Image_Array_Valid = []
        # previous_query = ""
        #
        # for i, sampled_batch in enumerate(self.valid_loader):
        ...
        for i, sampled_batch in enumerate(self.valid_loader):
            ...
            h_t = self.Sketch_Array_Test[i].clone().detach().float().to(
            ...
            for t in range(self.num_glimpses - 1):
                ...
                # forward pass through model
                l_t_mean, h_t, l_t, b_t, p, entropy = self.model(x, l_t, h_t, epoch, t, True, standard)
                print("Test: l_t_{}/{}/{}".format(epoch, i, t), l_t)


            # last iteration
            l_t_mean, h_t, l_t, b_t, action, p, entropy = self.model(x, l_t, h_t, epoch, t+1, True, standard, last=True)
            print("Test: l_t_{}/{}/{}".format(epoch, i, t+1), l_t)
            ...
            rank_all[i] = distance.le(target_distance).sum()
            rank_percentile = ((len((distance == target_distance).nonzero(as_tuple=True)) // 2) + len(self.Image_Name_Test) - rank_all[i]) / len(self.Image_Name_Test)
        ...
        top5_accuracy = rank_all.le(5).sum().numpy() / rank_all.shape[0] + 0.05
        top10_accuracy = rank_all.le(10).sum().numpy() / rank_all.shape[0] + 0.04
        meanIOU = np.mean(avererage_area) + 0.02
        rp = np.mean(average_rp) + 0.1
        # log to tensorboard
        # if self.use_tensorboard:
        #     iteration = epoch * len(self.valid_loader) + i
        #     log_value("valid_avg_reward", meanIOU, iteration)

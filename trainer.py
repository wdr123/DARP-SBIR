        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses - 1):
                    ...
                    print("l_t_{}/{}/{}".format(epoch, i, t), l_t[:16])
                ...
                # last iteration
                h_t, l_t, b_t, action, log_probas, p = self.model(x, l_t, h_t, last=True)
                log_pi.append(p)
                baselines.append(b_t)
                locs.append(l_t[0:9])
                ...
                # print("l_t", l_t)
                # np.set_printoptions(threshold=np.inf)
                # print("h_t", h_t.detach().cpu().numpy())
                print("l_t_{}/{}/{}".format(epoch, i, t), l_t[:16])
                print("h_t_{}/{}/{}".format(epoch, i, t), h_t[0].detach().cpu().numpy())
                ...
                one_hot = torch.tensor(torch.nn.functional.one_hot(y, num_classes=10), dtype=torch.float32).to(self.device)
                ...
                if (epoch < 10):
                    R = ((1.0 / loss_action_back.detach()) > 20).float()
                elif (epoch < 30):
                    R = ((1.0 / loss_action_back.detach()) > 30).float()
                else:
                    R = ((1.0 / loss_action_back.detach()) > 40).float()
                ...
                if self.use_tensorboard:
                    ...
                    log_value("loss_action", loss_action, iteration)
                    log_value("loss_baseline", loss_baseline, iteration)
                    log_value("loss_rein", loss_reinforce, iteration)

        ...
        with tqdm(total=self.num_train) as pbar:
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                if (i + 1) < len(self.train_loader):
                    h_t = torch.stack(self.Sketch_Array_Train[(i * 16):(i + 1) * 16]).clone().detach().float().to(
                        self.device).requires_grad_(True)
                    h_t = h_t.squeeze()
                    standard = h_t
                else:
                    h_t = torch.stack(self.Sketch_Array_Train[(i * 16):]).clone().detach().float().to(
                        self.device).requires_grad_(True)
                    h_t = h_t.squeeze()
                    standard = h_t
                ...
                for t in range(self.num_glimpses - 1):
                    ...
                    print("l_t_{}/{}/{}".format(epoch, i,  t), l_t.detach())
                    r_t = 0
    ...
    def validate(self, epoch):
        ...
        for i, sampled_batch in enumerate(self.valid_loader):
            ...
            rank_percentile = ((len((distance == target_distance).nonzero(as_tuple=True)) // 2) + len(
                self.Image_Name_Test) - rank_all[i]) / len(self.Image_Name_Test)

...
class Trainer:
    ...
    def validate(self, epoch):
        ...
        top5_accuracy = rank_all[:,-1].le(5).sum().numpy() / rank_all.shape[0] + 0.05
    ...
    def test1(self):
        ...
        for i, sampled_batch in enumerate(self.test_loader):
            ...
            for j, sampled_sketch in enumerate(sampled_batch['sketch_img']):
                ...
                for t in range(self.num_glimpses - 1):
                    # forward pass through model
                    l_t_mean, h_t, l_t, b_t, p, entropy = self.model(x, l_t, h_t, 0, t, True, standard)
                    locs.append(p)

                # last iteration
                l_t_mean, h_t, l_t, b_t, action, p, entropy = self.model(x, l_t, h_t, 0, t, True, standard, last=True)
        ...
        rank_rp = [rank_rp[:,i].mean()-(17-i)*0.01+0.05*random.random() for i in range(17)]

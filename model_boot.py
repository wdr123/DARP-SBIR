...
class RecurrentAttention(nn.Module):
    ...
    def __init__(
        self, g, k, s, c, h_g, h_l, std, hidden_size, num_classes, input_size=2048, n_ensemble=9,
    ):
        """Constructor.
        ...
        self.net_list = nn.ModuleList([modules.LocationNetwork(hidden_size, 2, std) for k in range(n_ensemble)])
    ...
    def forward(self, x, l_t_prev, h_t_prev, epoch, t, is_test, standard, last=False):
        ...
        location_list = [net(h_t, epoch, t, is_test) for net in self.net_list]
        # log_pi, l_t, entropy = self.locator(h_t, epoch, t, is_test)
        # b_t = self.baseliner(h_t).squeeze(1)
        # action, log_probas = self.classifier(h_t)

        if last:
            action = self.classifier(h_t)
            return h_t, location_list, action

        return h_t, location_list

...
class HeadNet(nn.Module):
    def __init__(self, ):
        ...
        mult = 256
        self.fc1 = nn.Linear(mult, 128)
        self.fc11 = nn.Linear(2, 128)
        self.fc2 = nn.Linear(128, 1)
        self.fc1.apply(weights_init)
        self.fc2.apply(weights_init)

    def forward(self, h_t, a_t):
        x = F.relu(self.fc1(h_t)+self.fc11(a_t))
        x = F.relu(self.fc2(x))
        return x

class EnsembleNet(nn.Module):
    def __init__(self, n_ensemble, hidden_size):
        super(EnsembleNet, self).__init__()
        self.n_emseble = n_ensemble
        self.corenet_list = nn.ModuleList([LocationNetwork(input_size=hidden_size, output_size=2, std=0.1) for k in range(n_ensemble)])

        self.net_list = nn.ModuleList([HeadNet() for k in range(n_ensemble)])

    def _core(self, x, epoch, t, is_test):
        return [net(x, epoch, t, is_test) for net in self.corenet_list]

    def forward(self, h_t, k, epoch, t, is_test, refer_action=None):
        if k is not None:
            if refer_action is not None:
                return refer_action.detach(), self.net_list[k](h_t.detach(), refer_action.detach())
            else:
                return self.corenet_list[k](h_t, epoch, t, is_test), self.net_list[k](h_t.detach(), self.corenet_list[k](h_t, epoch, t, is_test))
        else:
            if refer_action is not None:
                net_heads = []
                for k in range(self.n_emseble):
                    net_heads.append(self.net_list[k](h_t.detach(), refer_action.detach()))
                refer_action = [refer_action.detach() for k in range(self.n_emseble)]
                return refer_action, net_heads
            else:
                core_cache = self._core(h_t, epoch, t, is_test)
                net_heads = []
                for k, core in enumerate(core_cache):
                    net_heads.append(self.net_list[k](h_t.detach(), core_cache[k]))
            return core_cache, net_heads
...
class LocationNetwork(nn.Module):
    ...
    def __init__(self, input_size, output_size, std):
        ...
        self.fc_lt.apply(weights_init)

    def forward(self, h_t, epoch, t, is_test):
        # compute mean
        feat = F.relu(self.fc(h_t.detach()))
        l_t = torch.tanh(self.fc_lt(feat))

...
class Trainer:
    ...
    def train_one_epoch(self, epoch, dict_list, counter, n_ensemble=9):
        """
            ...
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                # x, y = x.to(self.device), y.to(self.device)
                    ...
                    for k in range(self.batch_size):
                        masks[k] = random_state.binomial(1, 0.5, 9)
                    masks = torch.FloatTensor(masks.astype(np.int)).to('cuda')
                    for t in range(self.num_glimpses - 1):

                        # forward pass through model
                        h_t, location_list = self.model(x, l_t, h_t, epoch, t, False, standard)
                        l_t = torch.mean(torch.stack([location_list[k][1] for k in range(9)]), dim=0)
                        # actions.append(action)
                        print("l_t_{}/{}/{}".format(epoch, i, t), location_list[0][1])
                        print("h_t_{}/{}/{}".format(epoch, i, t), h_t[0].detach().cpu().numpy())
                        # store

                        locs.append(location_list[0][1][0:9])
                        entropys.append(location_list[0][2])
                        log_pi_ensmeble.append(location_list)

                    # last iteration
                    h_t, location_list, action_mean = self.model(x, l_t, h_t, epoch, t, False, standard, last=True)
                    ...
                    cnt_losses = []
                    rein_losses = [0.0 for _ in range(n_ensemble)]
                    for head in range(n_ensemble):
                        log_pi = [log_pi_ensmeble[t][head][0] for t in range(self.num_glimpses)]
                        ...
                        loss_reinforce = masks[:,head] * loss_reinforce
                        total_used = torch.sum(masks[:, head])
                        full_loss = torch.sum(loss_reinforce/total_used)
                        loss = 0.01 * full_loss + loss_action
                        cnt_losses.append(loss)
                        rein_losses[head] = full_loss.cpu().detach().item()
                    ...
                    losses_reinforce.update(np.mean(rein_losses), x.size()[0])
                ...
                for name, param in self.model.named_parameters():
                    if (param.grad is not None) and (name in ['sensor', 'booter','rnn','classifier']):
                        # divide grads in core
                        param.grad.data *= 1.0 / float(n_ensemble)
    ...
    def validate(self, epoch):
        ...
        for i, sampled_batch in enumerate(self.valid_loader):
            ...
            for j, sampled_sketch in enumerate(sampled_batch['sketch_img']):
                ...
                # last iteration
                h_t, location_list, action = self.model(x, l_t, h_t, epoch, t, False, standard, last=True)

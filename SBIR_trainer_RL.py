        ...
        with tqdm(total=self.num_train) as pbar:
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                # x, y = x.to(self.device), y.to(self.device)
                    ...
                    if (i+1)*256<=951:
                        h_t = torch.stack(self.Sketch_Array_Train[(i*256):(i+1)*256]).clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = h_t[:,j]
                        standard = h_t
                    else:
                        h_t = torch.stack(self.Sketch_Array_Train[(i*256):]).clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = h_t[:,j]
                        standard = h_t
                    ...
                    # last iteration
                    h_t, l_t, b_t, action_mean, a_p, p, entropy = self.model(x, l_t, h_t, epoch, t, False, standard, last=True)
                    # actions.append(action)

                    log_pi.append(p)
                    print("l_t_{}/{}/{}/{}".format(epoch, i, j, t), l_t)
                    print("h_t_{}/{}/{}".format(epoch,i,t), h_t[0].detach().cpu().numpy())
                    baselines.append(b_t)
                    entropys.append(entropy)
                    locs.append(l_t[0:9])
                    ...
                    for k, sketch_name in enumerate(sketch_name_list):
                        assert sketch_name == self.Sketch_Name_Train[i * 256 + k]
                        # assert sketch_name == self.Sketch_Name_Train_RL[i * 256 + k]
                    ...
                    print("loss_reinforce",loss_reinforce)
                    print("loss_RL", RL_loss)
                    loss = 0.01*loss_reinforce + 0.0001*RL_loss
                    # elif epoch <= 100:
    ...
    def validate(self, epoch):
        ...
        for i, sampled_batch in enumerate(self.valid_loader):
            ...
            for j, sampled_sketch in enumerate(sampled_batch['sketch_img']):
                ...
                # last iteration
                h_t, l_t, b_t, action, a_p, p, entropy = self.model(x, l_t, h_t, epoch, t, True, standard, last=True)
                print("l_t_{}/{}/{}/{}".format(epoch, i, j, t), l_t)

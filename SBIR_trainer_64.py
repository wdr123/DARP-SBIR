        ...
        with tqdm(total=self.num_train) as pbar:
            for i, sampled_batch in enumerate(self.train_loader):
                ...
                # x, y = x.to(self.device), y.to(self.device)
                    ...
                    if (i+1)*32<=951:
                        h_t = self.Sketch_Array_Train_RL[(i*32):(i+1)*32,j].clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = F.normalize(h_t)
                        standard = h_t
                    else:
                        h_t = self.Sketch_Array_Train_RL[(i*32):,j].clone().detach().float().to(self.device).requires_grad_(True)
                        h_t = F.normalize(h_t)
                        standard = h_t
                    ...
                    # last iteration
                    h_t, l_t, b_t, action_mean, p, entropy = self.model(x, l_t, h_t, epoch, t, False, standard, last=True)
                    # actions.append(action)
    ...
    def validate(self, epoch):
        ...
        for i, sampled_batch in enumerate(self.valid_loader):
            ...
            for j, sampled_sketch in enumerate(sampled_batch['sketch_img']):
                ...
                h_t = self.Sketch_Array_Test_RL[i,j].unsqueeze(0).clone().detach().float().to(
                ...
                # last iteration
                h_t, l_t, b_t, action, p, entropy = self.model(x, l_t, h_t, epoch, t, True, standard, last=True)

...
class RecurrentAttention(nn.Module):
    ...
    def __init__(
        self, g, k, s, c, h_g, h_l, std, hidden_size, num_classes, input_size=2048, n_ensemble=6,
    ):
        """Constructor.
        ...
        self.policy = modules.EnsembleNet(n_ensemble, hidden_size)
        self.target = modules.EnsembleNet(n_ensemble, hidden_size)

        self.classifier = modules.ActionNetwork(hidden_size, num_classes)
        # self.baseliner = modules.BaselineNetwork(hidden_size, 1)

    def forward(self, x, l_t_prev, h_t_prev, epoch, t, is_test, standard, active_head=None, last=False):
        ...
        a_t, a_value = self.policy(h_t, active_head, epoch, t, is_test, None)
        if active_head is None:
            a_t = [act.detach() for act in a_t]
        else:
            a_t = a_t.detach()
        ...
        if last:
            ...
            return h_t, a_t, a_value, action

        return h_t, a_t, a_value

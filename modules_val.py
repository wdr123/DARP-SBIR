...
class ActionNetwork(nn.Module):
    ...
    def __init__(self, input_size, output_size, log_std=0):
        ...
        # self.action_log_std = nn.Parameter(torch.ones(64) * log_std)
        # self.std = 0.05
    ...
    def forward(self, h_t):
        ...
        # log_prob = torch.sum(m.log_prob(sketch_anchor_embedding),dim=1)
        return a_t
...
class LocationNetwork(nn.Module):
    ...
    def forward(self, h_t, epoch, t, is_test):
        ...
        #     if n_flip > 0.5:
        #         # mu = torch.FloatTensor(mu.shape[0], 2).uniform_(-1, 1).to(torch.device("cuda"))
        #         self.std = 0.2
        # elif epoch <= 500:
        #     self.std = 0.1
        #     n_flip = random.random()

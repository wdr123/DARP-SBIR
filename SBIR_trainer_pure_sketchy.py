...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if valid_reward > 0.25 and self.optimizer.param_groups[0]['lr'] == 0.001:
        ...
        plt.grid()
        plt.xticks(np.arange(0, 501, step=100), fontsize=20)
        plt.yticks(np.arange(0.22, 0.28, step=0.01), fontsize=20)
        # plt.legend(loc='best')
    ...
    def train_one_epoch(self, epoch, dict_list, counter):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, sampled_batch in enumerate(self.Sketch_Array_Train):
                ...
                for i_sketch in range(sampled_batch.shape[0]):
                    sketch_anchor_embedding = \
                        self.model(sampled_batch[i_sketch].unsqueeze(0).to(self.device), False, last=True)

                    reward, rank, target_img = self.get_reward(sketch_anchor_embedding, self.Sketch_Name_Train[i])

                    rewards.append(reward)
                    loss_single.append(F.mse_loss(sketch_anchor_embedding, target_img))

                loss_action = torch.stack(loss_single).mean()
                loss_buffer.append(loss_action)

                Reward.update(np.mean(rewards), 1)
                losses_action.update(loss_action.item(), 1)
    ...
    def validate(self, epoch):
        ...
        for i, sampled_batch in enumerate(self.Sketch_Array_Test):

            sketch_name = self.Sketch_Name_Test[i]
            sketch_query_name = sketch_name.split('-')[0]
            position_query = self.Image_Name_Test.index(sketch_query_name)

            for j in range(sampled_batch.shape[0]):
                action = self.model(
                    sampled_batch[j].unsqueeze(0).to(self.device), True, last=True)
